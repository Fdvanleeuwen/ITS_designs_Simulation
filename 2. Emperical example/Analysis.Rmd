...
```{r}
SR_out_1 <- lm(score ~ 1 + time + cohort + I(time^2) + corona + time:corona, data = cohort_17_18_19_long)
SR_out_2 <- lm(score ~ 1 + time*corona +corona*cohort + time:corona:cohort + I(time^2), data = cohort_17_18_19_long)
...
data_pred = data.frame(time = c(c(-2.13, -1.77, -1.14, -0.77, -0.13,  0.26,  0.99,  1.25), c(-1.13, -0.77, -0.14,  0.26,  0.99,  1.25,  1.89,  2.24)),
                      corona = c(c(0,0,0,0,0,1,1,1),c(0,0,0,1,1,1,1,1)),
                      cohort = as.factor(c(rep("17/18", 8),rep("18/19", 8))))
...
# predict new values
pred = predict(SR_out_1, newdata =data_pred)   
