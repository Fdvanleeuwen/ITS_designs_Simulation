...
```{r}
X <- forms_for_sim(N_time = 10, var = 10, Intercept = 170,  Slope = 16.5, step_size = -5, slope_size =  -5, additional_step = 0)

X
```

```{r, warning = F}
test <- gen_data(X$form, X$v_name, X$var, N_person = 100, X$Intercept, var_time = 20, additional_step_corr = 0)
...
#test$long <- test$long %>% 
  #mutate(treatment2 = as.factor(rep(c(0,0,0,1,2,2),100)),
        t#reatment3 = as.factor(rep(c(0,0,0,1,1,1),100)))
...
run_OLS(test$long, additional_step_corr = 0, time_2 = 1)
```
