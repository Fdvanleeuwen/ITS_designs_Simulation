...
source("functions/5.Chat_GPT_code.R")
```
...
```{r}
X <- forms_for_sim(N_time = 20, var = 10, Intercept = 170,  Slope = 33, step_size = -10, slope_size =  -10, additional_step = 0)
...
write.csv(out10, "results/sim10_3.csv")
write.csv(out01, "results/sim01_3.csv")
write.csv(out11, "results/sim11_3.csv")
```

```{r}
# One negative parameter
out11_neg_step <- sim_all(N_person, N_time, N_sim, var, Intercept, Slope, seed, var_time, ES, treatment_step = -1, treatment_slope = 1,additional_step = 0, additional_step_corr = 0,  model = "OLS")

out11_neg_slope <- sim_all(N_person, N_time, N_sim, var, Intercept, Slope, seed, var_time, ES, treatment_step = 1, treatment_slope = -1,additional_step = 0, additional_step_corr = 0,   model = "OLS")
```

```{r}
# write away the results
write.csv(out11_neg_step, "results/out11_neg_step_3.csv")
write.csv(out11_neg_slope, "results/out11_neg_slope_3 .csv")
```
...
```{r}
# write away the results
write.csv(out11_additional_step, "results/out11_additional_step_3.csv")
write.csv(out11_additional_step_corr, "results/out11_additional_step_corr_3.csv")
```
...
startTime <- Sys.time()
out10 <- sim_all_GPT(N_person, N_time, N_sim, var, Intercept, Slope, seed, var_time, ES, treatment_step = -1, treatment_slope = 0,additional_step = 0, additional_step_corr = 0,  model = "OLS")
endTime <- Sys.time()
print(endTime - startTime)
```
